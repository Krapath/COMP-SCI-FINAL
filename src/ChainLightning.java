...
public class ChainLightning extends GameObject {
    ...
    int damage = 1;
    int chainCount = 3;
    int chainRange = 100; // the amount of frames for each act
    ...
    public void paint(Graphics g) {
        ...
        for (int i = 0; i < enemyDisplay.size() - 1; i++) {
            ...
            if (distance > 20){
                ...
                int randomness = 10; // the maximum distance of the random offset, can be adjusted for more or less randomness
                
                // TODO: decide on two types of lightning
                
                //#1 random offset that changes every frame
                ...
                //#2  random offset that is static for the duration of the lightning
                int offsetX = (int) (Math.cos(randomAngleStatic) * randomness); // static offset in the x direction 
                int offsetY = (int) (Math.sin(randomAngleStatic) * randomness); // static offset in the y 
