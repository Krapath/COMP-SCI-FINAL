...
public class SpawnAnimation extends GameObject {
    ...
    public SpawnAnimation(PolygonGame game, int x, int y) {
        this.game = game;
        size = (int)((game.getWindowWidth() + game.getWindowHeight()) / 100);//THIS CODE HIGHLY NEEDS TO BE TWEAKED IF PLAYER SZIE CHANGED
        setSize(size, size);
    }
    public void spawnAnimation(){
        ...
        SpawnAnimation rightMiddle = new SpawnAnimation(game, game.getWindowWidth()+size, (game.getWindowHeight()+size)/2);
        ...
        SpawnAnimation bottomMiddle = new SpawnAnimation(game, (game.getWindowWidth()+size)/2, game.getWindowHeight()+size);
        game.add(bottomMiddle);
        spawnParticles.add(bottomMiddle);
        //add particle in bottom right
        SpawnAnimation bottomRight = new SpawnAnimation(game, game.getWindowWidth()+size, game.getWindowHeight()+size);
