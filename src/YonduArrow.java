...
public class YonduArrow extends GameObject {
    ...
    public void act() {
        ...
        if (rotationTimer > 0) {
            --rotationTimer;
            angle += rotateSpeed;
            // keep arrow centered on player while rotating, but rotate to face the closest enemy
            ...
            } else if (closestTarget != null) { // if the target was destroyed while aiming find a new target, if there are no enemies left just keep facing the same direction
                closestTarget = null;
                double closestDist = Double.MAX_VALUE;
                for (Enemy e : game.enemies) {
                    double dist = Math.sqrt(Math.pow(e.getX() - arrowCX, 2) + Math.pow(e.getY() - arrowCY, 2));
            ...
            if (returning) {
                ...
                if (arrowCX > game.player.getX() - size && arrowCX < game.player.getX() + Player.size + size &&
                    ...
                    //
                    angle = Math.atan2(
                            arrowCY - (game.player.getY() + Player.size / 2.0),
                            arrowCX - (game.player.getX() + Player.size / 2.0));
                    returning = false;
